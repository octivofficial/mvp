...
`;
  ...
  /**
   * Generate a Mermaid skill topology graph from the Zettelkasten.
   * Writes to vault/04-Skills/Skill-Graph.md for Obsidian rendering.
   */
  async _generateMermaidGraph() {
    const graph = await this.buildGraph();
    const nodes = Object.values(graph.nodes);

    if (nodes.length === 0) {
      log.info('got', 'No skills to graph');
      return;
    }

    const tierEmoji = {
      Novice: '🌱', Apprentice: '🔨', Journeyman: '⚒️',
      Expert: '⚔️', Master: '👑', Grandmaster: '🏆',
    };

    let mermaid = 'graph TD\n';

    for (const node of nodes) {
      const emoji = tierEmoji[node.tier] || '🌱';
      const rate = (node.successRate * 100).toFixed(0);
      const label = `${emoji} ${node.id} XP:${node.xp} ${rate}%`;
      mermaid += `  ${node.id}["${label}"]\n`;
    }

    for (const edge of graph.edges) {
      const label = edge.weight >= 0.7 ? '⚡' : edge.weight >= 0.4 ? '→' : '···';
      mermaid += `  ${edge.from} --"${label}"--> ${edge.to}\n`;
    }

    mermaid += '\n  classDef novice fill:#2d5016,stroke:#4ade80\n';
    mermaid += '  classDef expert fill:#7c2d12,stroke:#f97316\n';
    mermaid += '  classDef master fill:#7c3aed,stroke:#a78bfa\n';

    for (const node of nodes) {
      const tier = node.tier || 'Novice';
      if (tier === 'Expert' || tier === 'Master' || tier === 'Grandmaster') {
        mermaid += `  class ${node.id} ${tier === 'Grandmaster' ? 'master' : tier.toLowerCase()}\n`;
      } else {
        mermaid += `  class ${node.id} novice\n`;
      }
    }

    const date = new Date().toISOString().slice(0, 10);
    const md = `---\ntags: [graph, skills, auto-generated]\nupdated: "${date}"\n---\n\n# Skill Topology\n\n> Auto-generated by GoT Reasoner after each reasoning cycle.\n\n\`\`\`mermaid\n${mermaid}\`\`\`\n\n---\n*Last updated: ${new Date().toISOString()}*\n`;

    const graphPath = path.join(this.vaultDir, '..', 'Skill-Graph.md');
    await fsp.writeFile(graphPath, md, 'utf-8');
    log.info('got', 'Skill graph updated');
  }
