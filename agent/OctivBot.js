...
class OctivBot {
    ...
    async start() {
        try {
            ...
            console.log(`[Blackboard] Connected: ${this.options.redisUrl}`);
        } catch (err) {
    ...
    async _onHealthChange() {
        if (this.bot.health <= 10) {
            console.warn(`⚠️  Low health warning: ${this.bot.health}/20`);
        }
        ...
        try {
            await this.redisClient.set('octiv:bot:health:latest', JSON.stringify(data), { EX: 300 });
        } catch (err) {
            // Ignore Redis errors inside events to allow resilience
        }
    }
    ...
    async _publishStatus(status) {
        ...
        try {
            await this.redisClient.set('octiv:bot:status:latest', JSON.stringify(data), { EX: 300 });
            await this.redisClient.publish('octiv:blackboard', JSON.stringify(data));
        } catch (err) {
